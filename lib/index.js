...
Intruder.prototype.crack = function(ssid, done) {
  ...
  discover(function(err, networks) {
    ...
    channel = channel || network[2];
    var bssid = network[1];

    self.sniff(channel, function(file) {
      // run `aircrack-ng -1 -a 1 -b MAC airportSniff*.cap` if `interval` has passed
      // exec('aircrack-ng -1 -a 1 -b ' + network[1] + ' ' + file, function(err, res) {
      //
      // });
    });
  });
...
Intruder.prototype.sniff = function(channel, done) {
